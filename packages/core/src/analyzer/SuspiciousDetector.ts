...
export class SuspiciousDetector {
    private indicators: SuspiciousIndicator[] = [];
    ...
    calculateRiskScore(indicators: SuspiciousIndicator[]): number {
    ...
    getRiskLevel(score: number): SuspiciousSeverity {
    ...
    groupByBlock(transactions: Transaction[]): SameBlockGroup[] {
    ...
    private detectRapidMovement(transactions: Transaction[]): void {
        ...
        for (const incoming of incomingByTime) {
            ...
            const matchingOutgoing = outgoingByTime.find(
                out => out.timestamp > incoming.timestamp &&
    ...
    private detectSameBlockActivity(transactions: Transaction[]): void {
        ...
        if (suspiciousBlocks.length > 0) {
            ...
            this.indicators.push({
                ...
                evidence: suspiciousBlocks.slice(0, 3).map(
                    b => `Block ${b.blockNumber}: ${b.transactions.length} transactions`
                ),
    ...
    private detectCircularFlow(transactions: Transaction[]): void {
    ...
    private detectDustAttacks(transactions: Transaction[]): void {
        const dustTxs = transactions.filter(
            tx => tx.isIncoming && tx.valueInEth > 0 && tx.valueInEth < 0.0001
        );
        ...
        if (dustTxs.length >= 5) {
            this.indicators.push({
                ...
                evidence: dustTxs.slice(0, 3).map(
                    tx => `${tx.valueInEth.toFixed(8)} ETH from ${tx.from.slice(0, 10)}...`
                ),
                score: 10,
            });
        }
    ...
    private detectFreshWallet(walletAge?: number, transactions?: Transaction[]): void {
        if (!walletAge || !transactions) return;
    ...
    private detectWashTrading(transactions: Transaction[]): void {
        // Look for repeated same-value transactions between same parties
        const txPairs = new Map<string, number>();
        ...
        if (repeatedPairs.length > 0) {
            ...
            this.indicators.push({
                ...
                evidence: repeatedPairs.slice(0, 3).map(
                    ([key, count]) => `${count}x identical transfers`
                ),
    ...
    private flattenChildren(node: FundingNode): FundingNode[] {
        const result: FundingNode[] = [];
